  ...
  while ((c = getopt (argc, argv, "i:o:h")) != -1)
    {
      switch (c)
        ...
        case 'i':
          opt->input_flag = true;
          opt->input_arg = (char *)malloc (strlen (optarg) + 1);
          memcpy (opt->input_arg, optarg, strlen (optarg) + 1);
        ...
        case 'o':
          ...
          opt->output_arg = (char *)malloc (strlen (optarg) + 1);
          memcpy (opt->output_arg, optarg, strlen (optarg) + 1);
          if (strcmp (optarg, "stdio")
              && (!atoi (optarg) || atoi (optarg) < 0))
            {
